...
| Product | Status |
|---|---|
| **BigQuery** | **Not used.** `services/analytics_service.py` is written as single grouped aggregations over a bounded window — the exact shape the equivalent SQL would take — but the executor is Python over Firestore. At a few hundred records the Python path returns in milliseconds and costs nothing; at real volume it would not. This is the most obvious next piece of work. |
| **MCP Toolbox for Databases** | **Not used.** It was in the brief, and pairing it with BigQuery would give a genuine feature ("which vendors should we audit this quarter, and why"). Not built. |
